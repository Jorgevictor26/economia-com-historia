...
@Component({
  ...
  template: `
    <section class="-m-6 min-h-dvh bg-[#191c1d] text-white">
      <app-public-navbar />

      <main class="relative min-h-[calc(100dvh-58px)] overflow-hidden bg-[radial-gradient(circle_at_50%_50%,#2e3132_0%,#191c1d_64%)]">
        ...
        <section class="relative z-10 grid min-h-[calc(100dvh-58px)] gap-5 px-4 py-5 lg:grid-cols-[320px_minmax(0,1fr)_384px] lg:px-6">
          <aside class="map-glass order-2 rounded-[8px] p-5 lg:order-1 lg:max-h-[calc(100dvh-160px)]">
            <div class="relative">
              <input class="w-full border-0 border-b border-[#d8c1c4]/35 bg-white/8 py-3 pl-10 pr-3 text-sm text-white outline-none placeholder:text-white/45 focus:border-[#ffb1c0]" placeholder="Pesquisar local..." type="text" />
            ...
            <div class="mt-6 grid gap-6">
              <section>
                ...
                <div class="mt-3 grid gap-3">
                  <select class="w-full border-0 border-b border-white/20 bg-transparent py-2 text-sm text-white outline-none">
                ...
                <div class="mt-3 grid gap-2">
                  @for (category of categories; track category.label) {
                    <button type="button" class="flex items-center gap-3 rounded-[8px] p-2 text-left text-sm text-white/80 transition hover:bg-white/10 hover:text-white">
                      <span class="material-symbols-outlined" [class.text-[#ffb1c0]]="category.tone === 'primary'" [class.text-[#ffe088]]="category.tone === 'gold'" [class.text-[#8be0d4]]="category.tone === 'teal'" aria-hidden="true">{{ category.icon }}</span>
                      {{ category.label }}
                    </button>
                  }
                </div>
            ...
            <div class="mt-6 border-t border-white/15 pt-4">
              <div class="flex items-center justify-between rounded-[8px] border border-[#904859]/40 bg-[#5c1e2f] p-3">
                ...
                <span class="inline-flex items-center gap-2 text-xs text-white/82">
          ...
          <section class="order-1 min-h-[560px] lg:order-2">
            <div class="map-glass mx-auto mb-5 flex w-fit items-center gap-1 rounded-full p-1">
              <button type="button" class="rounded-full bg-[#40081a] px-5 py-2 text-xs font-bold text-white">Visão econômica</button>
            ...
            <div class="relative flex min-h-[480px] items-center justify-center">
              ...
              @for (point of points; track point.id) {
                ...
                >
                  <span class="material-symbols-outlined text-[32px] transition group-hover:scale-125" [class.text-[#ffb1c0]]="point.tone === 'primary'" [class.text-[#ffe088]]="point.tone === 'gold'" [class.text-[#8be0d4]]="point.tone === 'teal'" aria-hidden="true">{{ point.icon }}</span>
                  <span class="pointer-events-none absolute top-9 whitespace-nowrap rounded-[6px] border border-white/15 bg-[#5c1e2f] px-3 py-1 text-[11px] font-bold opacity-0 shadow-xl transition group-hover:opacity-100">{{ point.label }}</span>
                </button>
              }
            </div>
          ...
          <aside class="map-glass order-3 rounded-[8px] p-6 lg:max-h-[calc(100dvh-160px)]">
            @if (selectedPoint(); as point) {
              ...
              <section class="mt-5">
                <h3 class="text-[11px] font-extrabold uppercase tracking-[0.16em] text-[#ffb1c0]">Indicadores econômicos</h3>
                ...
                <div class="mt-4 flex h-24 items-end gap-2 px-2">
                  <i class="flex-1 rounded-t-sm bg-[#5c1e2f] h-1/2"></i>
                  <i class="flex-1 rounded-t-sm bg-[#5c1e2f] h-2/3"></i>
                  <i class="flex-1 rounded-t-sm border border-[#ffb1c0] bg-[#904859] h-5/6"></i>
                  <i class="flex-1 rounded-t-sm bg-[#5c1e2f] h-full"></i>
                </div>
              </section>
        ...
        <div class="pointer-events-none fixed bottom-6 right-6 z-30 hidden flex-col gap-3 lg:flex">
          <button type="button" class="pointer-events-auto inline-flex items-center gap-3 rounded-full bg-[#735c00] px-6 py-3 text-sm font-extrabold text-white shadow-2xl transition hover:scale-105" (click)="selectPoint(points[1])">
            <span class="material-symbols-outlined" aria-hidden="true">explore</span>
            Explorar História
          </button>
          <button type="button" class="pointer-events-auto inline-flex items-center gap-3 rounded-full bg-[#40081a] px-6 py-3 text-sm font-extrabold text-white shadow-2xl transition hover:scale-105" (click)="selectPoint(points[0])">
  ...
  readonly points: MapPoint[] = [
    {
      ...
      history: 'Fundada em 1576 por Paulo Dias de Novais, Luanda tornou-se um centro administrativo, portuário e financeiro. O seu crescimento foi impulsionado pelo porto natural, pelos serviços e pela economia petrolífera.',
      stats: [
    ...
    },
    {
      ...
      history: 'O Huambo articulou agricultura, caminhos-de-ferro e circulação de pessoas no planalto central. A região ajuda a compreender a ligação entre infraestrutura, produção e memória social.',
      stats: [
    ...
    },
    {
      ...
      history: 'O corredor liga zonas mineiras e agrícolas ao Atlântico, tornando-se uma peça central na discussão sobre exportações, integração regional e novas geografias econômicas.',
      stats: [
